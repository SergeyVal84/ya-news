...
def test_user_can_create_comment(author_client, author, form_data, news):
    ...
    assertRedirects(
        response,
        reverse('news:detail', kwargs={'pk': news.pk}) +'#comments'
    )
...
def test_author_can_edit_only_his_comments(
        author_client, news, comments, form_data
    ):
    url = reverse('news:detail', kwargs={'pk': news.pk})
...
def test_author_can_edit_only_his_comments(
        not_author_client, news, comments, form_data
    ):
    url = reverse('news:detail', kwargs={'pk': news.pk})
...
def test_author_can_delete_comments(author_client, news, comments):
    ...
    assertRedirects(
        response,
        reverse('news:detail', kwargs={'pk': comments.news.pk}) + '#comments'
    )
    assert Comment.objects.count() == 0
